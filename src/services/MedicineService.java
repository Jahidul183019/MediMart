...
public class MedicineService {
    ...
    // ✅ Get All Medicines (throws AppException on failure; snapshots to file on success;
    //    falls back to snapshot file if DB read fails)
    public List<Medicine> getAllMedicines() {
        String sql = "SELECT id,name,category,price,quantity,expiry,image_path FROM medicines";
    ...
    // ✅ Get Medicine by ID (throws AppException on DB failure; returns null if not found)
    public Medicine getMedicineById(int id) {
        String sql = "SELECT id,name,category,price,quantity,expiry,image_path FROM medicines WHERE id=?";
        try (Connection conn = DBHelper.getConnection();
    ...
    // ✅ Add Medicine (persists image_path; stamps last_updated)
    ...
    // ✅ Add & return generated ID (optional helper)
    ...
    // ✅ Update (without touching image_path)
    public boolean updateMedicine(int id, String name, String category, double price, int quantity, String expiry) {
        String sql = """
                UPDATE medicines
        ...
        try (Connection conn = DBHelper.getConnection();
            ...
            try { appendFailsafe("update", new Medicine(id, name, category, price, quantity, expiry, null)); }
    ...
    public boolean updateMedicine(Medicine m) {
        ...
        try (Connection conn = DBHelper.getConnection();
            ...
            FileLogger.error("DB error updateMedicine(with image) id=" + (m != null ? m.getId() : null) + ": " + e.getMessage(), e);
    ...
    public boolean updateQuantity(int id, int qty) {
        ...
        try (Connection conn = DBHelper.getConnection();
            ...
            try { appendFailsafe("updateQty", new Medicine(id, null, null, 0.0, qty, null, null)); }
            catch (Exception ioEx) { FileLogger.warn("Failsafe log failed: " + ioEx.getMessage()); }
    ...
    public boolean updateImagePath(int id, String imagePath) {
        ...
        try (Connection conn = DBHelper.getConnection();
            ...
            try { appendFailsafe("updateImage", new Medicine(id, null, null, 0.0, 0, null, imagePath)); }
            catch (Exception ioEx) { FileLogger.warn("Failsafe log failed: " + ioEx.getMessage()); }
    ...
    public boolean deleteMedicine(int id) {
        ...
        try (Connection conn = DBHelper.getConnection();
            ...
            try { appendFailsafe("delete", new Medicine(id, null, null, 0.0, 0, null, null)); }
            catch (Exception ioEx) { FileLogger.warn("Failsafe log failed: " + ioEx.getMessage()); }
    ...
    private static final Path SNAPSHOT_FILE = Paths.get("data", "medicines_snapshot.json");
    private static final Path OFFLINE_QUEUE = Paths.get("data", "offline_queue.jsonl");
    ...
    private void writeSnapshot(List<Medicine> list) throws Exception {
        ...
        sb.append("{\"savedAt\":").append(Instant.now().getEpochSecond()).append(",\"items\":[");
    ...
    private List<Medicine> readSnapshot() throws Exception {
        ...
        // split by top-level commas (no nested objects here other than flat Medicines)
        int brace = 0;
    ...
    private static double doubleVal(String s, String key, double def) {
        ...
        int j = i;
        while (j < s.length() && "-0123456789.".indexOf(s.charAt(j)) >= 0) j++;
        try { return Double.parseDouble(s.substring(i, j)); } catch (Exception e) { return def; }
    }
    ...
    private void tryBroadcastRefresh() {
        // Don’t make the whole service depend on the optional socket class.
        try {
            ...
            // Any other reflection/runtime error – log and continue
